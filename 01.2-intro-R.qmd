...
#| fig-alt: Screenshot of the R console opening screen after installation, showing the command prompt ready for input.
knitr::include_graphics("img/mod01/R-screenshot.png")
```

Near the bottom of the R screen, you will find the "\>" symbol which represents R's command line; this is where you can type your R commands for processing. If you type `1 + 2` into the command line and then hit enter you should get:
...
#| fig-alt: Screenshot of the RStudio interface with the main panes visible.
knitr::include_graphics("img/mod01/RStudio-screenshot-01.png")
```

Locate the R's command line symbol "\>" at the bottom of the left-hand panel. Type `1 + 2` into the command line and hit enter, and you will see:
...
Make sure you are in the **General** pane, and **untick "Restore .RData into workplace at startup"**, and choose: "**Save workspace to .RData on exit:** **Never"**.

```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot of the RStudio preferences window where workspace restoration settings can be changed.
...
Step 1: Choose **File \> New Project...** in RStudio to open the **Create Project** dialog box. Click the first option to create a project in a **New directory**:

```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot of RStudio’s Create Project window with the New Directory option selected.
knitr::include_graphics("img/mod01/NewProject-1.png")
```

Step 2: Click the first option: **New Project**:

```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot of the next step in the project wizard with New Project selected.
knitr::include_graphics("img/mod01/NewProject-2.png")
```

Step 3: Give the project a name, by typing PHCM9795 in the "Directory name", and choose where you want to store the project by clicking the **Browse** button. Finally, click **Create** to create your project.

```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot of the project wizard where the project is named PHCM9795 and a folder location is selected.
...
```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot showing the newly created PHCM9795 project and the RStudio panes after creation.
...
Create a new project called PHCM9795. Store it in a folder that you will remember, so that you can store all your R work in the same folder over the term.
:::
...
```{r echo=FALSE, fig.show="hold", out.width="100%"}
#| fig-alt: Screenshot showing a simple R script typed into the script editor, including comments and summary(age).
knitr::include_graphics("img/mod01/RStudio-screenshot-02.png")
```

To run your script, choose **Code \> Run Region \> Run All** from the menu bar. You will see your code appear in the bottom-left window, with the following output:
...
```{r echo=FALSE, fig.show="hold", out.width="100%"}
#| fig-alt: Screenshot of the RStudio environment after running a script, showing the four standard panes.
...
Notice the tab at the top of the Source window which includes the name of the current file. The name of the script will be black when it has been saved, and will change to red if you have any unsaved changes.
...
If you do some reading about R, you may learn that R is based on **objects** and **functions**. We can think of objects as the 'nouns' of R, and functions as the 'verbs'. When we enter data, or import data into R, we are asking R to create **objects** from our data. These objects can be manipulated and transformed by **functions**, to obtain useful insights from our data.
...
We created an object earlier, called `age`, which comprised six values. Let's see another example:
...
This example is rather trivial, and we rarely assign objects of just one value.
...
There are other structures in R, such as matrices and lists, which we won't discuss in this course. You may also come across the term **tibble**, which is a type of data frame.

#### Functions

As the 'verbs' of R, functions transform objects. Functions can transform your data into summary statistics, graphical summaries or analysis results. For example, we used the `summary()` function to display summary statistics for our six ages.
...
```{r echo=FALSE, fig.show="hold", out.width="80%"}
#| fig-alt: Screenshot of the R help page showing usage details for the summary function.
...
```{r echo=FALSE, fig.show="hold", out.width="80%"}
#| fig-alt: Screenshot of the bottom of an R help page showing example usage code.
knitr::include_graphics("img/mod01/help-2.png")
```

The `summary()` function is quite simple, in that it only requires one input, the object to be summarised. More complex functions might require a number of inputs. For example, the help notes for the `descriptives()` function in the `jmv` package show a large number of inputs can be specified. Instructions for installing the `jmv` package will be provided later, this help-screen is included for illustration only.

```{r echo=FALSE, fig.show="hold", out.width="80%"}
#| fig-alt: Screenshot of the help page for the descriptives function with multiple arguments listed.
...
#| fig-alt: Screenshot of the RStudio package installation window with a search field and suggested packages.
...
Note that quotation marks are not required for the `library()` function (although they can be included if you really like quotation marks!). Another example of R's inconsistency!!!
...
Packages in the tidyverse have been designed with a goal to make using R more consistent by defining a "grammar" to manipulate data, examine data and draw conclusions from data. The tidyverse is a powerful set of packages, and we will use only a small part of its capabilities.

More information about the tidyverse can be found using freely available resources such as: [Tidyverse Skills for Data Science](https://jhudatascience.org/tidyversecourse/) and [R for Data Science](https://r4ds.had.co.nz/).

## Part 2: Obtaining summary statistics for continuous data

In this exercise (spanning Modules 1 and 2), we will analyse data to complete a descriptive table from a research study. The data come from a study in primary biliary cirrhosis, a condition of the liver, from Modeling Survival Data: Extending the Cox Model by @therneau_grambsch10. By the end of this exercise, we will have completed some of @tbl-r-t1.
...
tt(tab, escape = TRUE, notes = "* aspartate aminotransferase") |>
  format_tt(replace = "")
```

This table is available in `Table1.docx`.

::: {.callout-note icon="false"}
## TASK

Download the table shell, saved as `PBC Table1.docx`, and the information file called `mod01_pbc_info.txt`.
:::
...
```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot showing the New Folder icon in the Files pane.
...
Download the "Data sets: for learning activities", and use Windows Explorer or MacOS Finder to save these data sets in **activities**. Save the "Data sets: example data from course notes" into the **examples** folder.

Your **activities** folder should look like:

```{r echo=FALSE, fig.show="hold", out.width="75%"}
#| fig-alt: Screenshot of the project's data folder after creating activities and examples subfolders.
...
#| fig-alt: Screenshot of the skimr summary output for the pbc dataset, including summary statistics and mini histograms.
...
```{r}
#| fig-alt: Density plots for three continuous variables
...
![](img/mod01/r-dens-01.png){fig-alt="Screenshot showing the density plots output in the RStudio Plot window with navigation arrows for browsing plots."}
...
```{r}
#| fig-alt: Density plot for age. The x-axis is labelled "N=418, Bandwidth=2.812", the y-axis is labelled "Density" and the main title is "density(x=pbc$age)"
...
```{r}
#| fig-alt: Density plot for age. The x-axis is labelled "Age in years", the y-axis is labelled "Density" and the main title is "Density plot of participant age from pbc study data"
plot(
  density(pbc$age),
  xlab = "Age in years",
...
```{r}
#| fig-alt: Boxplot for age showing a box with roughly equal whiskers.
boxplot(
  pbc$age,
  xlab = "Age (years)",
...
#| message: false
#| warning: false
#| fig-alt: Density plot of weights showing a small bump at an extreme high value, suggesting an unusual value. Boxplot of weights with one extreme point far above the rest of the data, indicating an unusual value.
sample <- readRDS("data/examples/mod01_weight_1000.rds")

par(mfrow = c(1, 2))
plot(
  density(sample$weight),
  xlab = "Weight (kg)",
...
#| message: false
#| warning: false
print(subset(sample, weight > 200))
```
