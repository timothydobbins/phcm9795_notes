...
```{r}
# Define "Active" as the first level of group and "Nausea" as the first level of side_effect:
drug <- mutate(drug, group = relevel(group, ref="Active"),
...
```{r}
drug_aggregated = mutate(drug_aggregated,
                         group = factor(group, levels=c("Active", "Placebo")),
                         side_effect = factor(side_effect, levels=c("Nausea", "No nausea")))
```
