...
#| fig-alt: Line chart showing the top five causes of death in Australia from 2014 to 2023, with age-standardised death rates changing over time.
...
The most commonly used measure of the central tendency of the data is the mean, calculated as the sum of all the values divided by the number of values:
...
Another measure of central tendency is the median. The median is the middle value of the data, the value at which half of the measurements lie above it and half of the measurements lie below it.
...
```{r fig-hist-1, echo=FALSE, out.width = "100%", fig.cap="Histograms of age and serum bilirubin from PBC data"}
#| fig-alt: Two histograms shown side by side, one for age with a roughly symmetric distribution and one for serum bilirubin with a strong right skew.
...
#| fig-alt: Two density plots comparing a roughly symmetric age distribution with a highly right-skewed serum bilirubin distribution.
...
-   any values in the dataset lying outside the whiskers are plotted individually, often labelled by the row number of the observation.

@fig-box-1 presents two example boxplots for age and serum bilirubin.

```{r fig-box-1, echo=FALSE, out.width = "100%", fig.cap="Box plot of age and serum bilirubin from PBC study data"}
#| fig-alt: Two boxplots comparing age and serum bilirubin, with serum bilirubin showing a longer upper tail and more extreme values.
