...
tt(select(tab10_1, -P), escape = TRUE) |> 
  tt_format(replace=" ") |> 
  tt_style(align="c", size=0.5) |> 
  group_tt(j = list("Required confidence interval width" = 2:11))
```
...
: Comparison of study result with the truth {#tbl-truth-vs-study-html}
...
tt(tab, cwidth = c(1.5, 1.5, 1.5, 1.5))
```
