...
tt(select(tab_out, 1:3)) |> 
  format_tt(
    j = 3,
    digits = 1, num_fmt = "decimal", escape=TRUE) |> 
  style_tt(j = 2:3, align = "dd")
```
...
tab_stage_out <- as.data.frame(tab_stage$frequencies[[1]]) |>
    ...
    mutate(
    `Relative frequency (%)` = formatC(100 * `Relative frequency (%)`, format="f", digits=1),
    `Cumulative relative frequency (%)` = formatC(100 * `Cumulative relative frequency (%)`, format="f", digits=1)
    )

tt(select(tab_stage_out, 1:4), 
   width = c(1, 1, 1.5, 1.5),
   notes = list(
     "*" = list(i = 0, j = 1, text = "Disease stage was missing for 6 participants"))) |>
...
df <- pbc |> 
  mutate(stage = factor(stage, levels=c(1,2,3,4), labels=c("Stage 1", "Stage 2", "Stage 3", "Stage 4"))) |> 
  rename(Sex = sex,
         `Stage of disease` = stage)

datasummary_crosstab(Sex ~ `Stage of disease`, data=df,
                     statistic = 1 ~ 1 + N,
