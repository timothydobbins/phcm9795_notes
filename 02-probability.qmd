...
tab_stage_out <- as.data.frame(tab_stage$frequencies[[1]]) |>
  ...
  mutate(
    ...
    `Cumulative relative frequency (%)` = sprintf("%.1f", 100 * `Cumulative relative frequency (%)`))
...
finaltable <- tt(ds, notes = c('* Stage of disease was missing for 6 participants'))
