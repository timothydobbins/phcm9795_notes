...
tt(select(tab_out, 1:3), escape = TRUE) |>
  tt_format(j = 3, digits = 1, num_fmt = "decimal") |>
  tt_style(align = "lcc")
```
...
#| tbl-pos: h
tab_stage <- descriptives(pbc, `Stage of disease`, freq = TRUE)
...
tab_stage_out <- as.data.frame(tab_stage$frequencies[[1]]) |>
  ...
  mutate(
    `Relative frequency (%)` = sprintf("%.1f", 100 * `Relative frequency (%)`),
    `Cumulative relative frequency (%)` = sprintf(
      "%.1f",
      100 * `Cumulative relative frequency (%)`
    )
  )

tt(select(tab_stage_out, 1:4), escape = TRUE) |>
  tt_style(align = "lccc")
```
...
datasummary_crosstab(
  ...
  data = pbc,
  statistic = 1 ~ 1 + N,
  notes = c('* Stage of disease was missing for 6 participants')
) |>
  format_tt(replace = list("Count" = "N"))
```
...
## Summarising two categorical variables graphically {#sec-two-catvar-graph}
...
knitr::include_graphics(here::here(
  "img",
  "mod02",
  "pbc-bar-stage-sex-stacked-relative.png"
))
```
...
tt(
  tab1_pres,
  width = c(2, 5),
  notes = c('Sources: Altman (1990), Cole (2015), Assel et al. (2019)')
)
```
...
We can use the [applet](https://mabognar.github.io/apps/bin.html){target="_blank"} to calculate this, or use the `dbinom` function in R with x=3, size=6, and prob=0.198. This gives an answer of 0.08.
...
We can use the [applet](https://mabognar.github.io/apps/bin.html){target="_blank"} to calculate this, or in R we can use the `pbinom` function with the `lower.tail=FALSE` option.
...
Again, we can use the [applet](https://mabognar.github.io/apps/bin.html){target="_blank"} to calculate this, or use the `pbinom` function in R.
