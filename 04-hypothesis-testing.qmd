...
df <- data.frame(
  ...
  y_min = c(1.2, 0.6, 0.1, -0.4, -0.2),
  y_max = c(2.2, 1.8, 0.9, 1.6, 0.8)
)

# 2. Generate the plot
ggplot(df, aes(x = label)) +
  # Add the error bars
  geom_errorbar(aes(ymin = y_min, ymax = y_max), width = 0.2, size = 0.8) +
  # Add the Null Hypothesis line (y = 0)
  geom_hline(yintercept = 0, linetype = "solid", size = 0.7) +
  # Add the "Important" threshold line (y = 1)
  geom_hline(yintercept = 1, linetype = "dashed", color = "black") +
  # Formatting scales and labels
  ...
  annotate(geom = "text", x = 2, y = 2.4, label = "Evidence of a difference") +
  annotate(
    geom = "text",
    x = 4.5,
    y = 2.4,
    label = "No evidence of a difference"
  ) +
  annotate(
    geom = "segment",
    x = 0.8,
...
: Comparison of study result with the truth {#tbl-mod4-alpha-beta}
...
: Summary of blood pressure from female Pima indigenous Americans
...
#| fig-alt: A symmetric bell-shaped t-distribution centred at zero. Both tails are shaded beyond the critical values, illustrating the rejection regions for a two-tailed hypothesis test with alpha equal to 0.05.
...
## Indigenous data sovereignty and governance: the Australian context
