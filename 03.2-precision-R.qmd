...
A common type of file that data are stored in is a Microsoft Excel file (.xls or .xlsx). In this demonstration, we will import a selection of records from a large health survey, stored in the file `mod03_health_survey.xlsx`.
...
g1 <- ggplot(data=survey, mapping=aes(x=bmi)) +
  geom_density() +
  theme_classic() +
  labs(x="BMI (kg/m2)")


g2 <- ggplot(data=survey, mapping=aes(y=bmi)) +
  geom_boxplot() +
  theme_classic() +
  labs(y="BMI (kg/m2)") +
  theme(
...
`dplyr` is a package that provides a set of functions to manipulate data. We have previously created new columns of data, or changed existing columns using `<-`. In `dplyr`, we can use the `mutate()` function to create new variables or to change existing variables. The advantage of `mutate()` is that we can create multiple new variables in the one function, using code that is easy to read.

For example, in this module we have used `<-` to:

1. define `sex` as a **factor**, and
2. create BMI.

We could do these manipulations using `mutate()` as:
...
survey <- mutate(survey, sex = factor(sex, levels=c(1,2), labels=c("Male", "Female")),
                         bmi = weight / height^2)

```

Note that the results of `<-` and `mutate()` are identical. Some authors, however, see the `mutate()` approach easier to read.
...
## Using `ggplot2` to plot density plots by another variable

We can define a grouping variable as a `facet_wrap` to create a grid of graphs separately for each value of the grouping variable. For example, to create a density plot for each sex, we would use `facet_wrap(vars(sex))` as follows:
...
## Using `ggplot2` to plot boxplots plots by another variable

As for density plots, we can use a `facet_wrap` for boxplots. Alternatively (and possibly easier), we can define `sex` as an `x` variable in our `mapping` statement:

```{r}
ggplot(data=survey, mapping=aes(y=bmi, x=sex)) +
  geom_boxplot() +
  theme_classic() +
  labs(y="BMI (kg/m2)")
```
...
The Normal Distribution Applet posted at [https://mabognar.github.io/apps/normal.html](https://mabognar.github.io/apps/normal.html){target="_blank"}, provides a simple and intuitive way to compute probabilities from a binomial distribution. Refer to the jamovi instructions for details.
